...
try:
    ...
    print("PyTorch/Torchvision not available - Deep Learning features disabled")
...
# ==========================================
# NEW: One-Shot Siamese Tracker Class
# ==========================================
class DeepSiameseTracker:
    def __init__(self, use_gpu=True):
        self.use_gpu = use_gpu and torch.cuda.is_available()
        self.device = torch.device('cuda' if self.use_gpu else 'cpu')
        
        if TORCH_AVAILABLE:
            # We use ResNet18 but cut it off early (at layer3)
            # This gives us better spatial resolution (stride 16) compared to the full network (stride 32)
            full_resnet = models.resnet18(pretrained=True)
            self.backbone = torch.nn.Sequential(
                full_resnet.conv1, full_resnet.bn1, full_resnet.relu, full_resnet.maxpool,
                full_resnet.layer1, full_resnet.layer2, full_resnet.layer3
            )
        ...
        self.stride = 16 # ResNet up to layer3 has a total stride of 16

    def init(self, frame, bbox):
        """
        One-Shot Learning Step: Extract features from the initial object.
        """
        self.last_bbox = bbox
        x, y, w, h = [int(v) for v in bbox]
        
        # Crop the template (the object)
        # We add a small context margin for better feature extraction
        roi = frame[y:y+h, x:x+w]
        
        # If roi is invalid, return
        if roi.size == 0: return

        # Extract features (Z)
        with torch.no_grad():
            img_tensor = self.transform(roi).unsqueeze(0).to(self.device)
            self.template_features = self.backbone(img_tensor)
            
        print(f"One-Shot Learning: Object features extracted. Shape: {self.template_features.shape}")

    def update(self, frame):
        """
        Search for the object in the new frame using Cross-Correlation.
        """
        ...
        # 1. Define Search Region (larger than the object, centered on last pos)
        # We search 3x the size of the object to capture movement
        padding_w = w 
        padding_h = h 
        
        search_x1 = max(0, x - padding_w)
        search_y1 = max(0, y - padding_h)
        search_x2 = min(frame.shape[1], x + w + padding_w)
        search_y2 = min(frame.shape[0], y + h + padding_h)
        ...
        # 2. Extract Search Features (X)
        with torch.no_grad():
            input_tensor = self.transform(search_crop).unsqueeze(0).to(self.device)
            search_features = self.backbone(input_tensor)

        # 3. Cross-Correlation (The "Matching" Step)
        # We use the template features as a convolution kernel
        # Output is a score map (heatmap)
        score_map = F.conv2d(search_features, self.template_features)
        
        # 4. Find location of max score
        # The score map is smaller than the image by factor of self.stride (16)
        _, _, max_y_feat, max_x_feat = torch.where(score_map == torch.max(score_map))
        
        max_x_feat = max_x_feat[0].item()
        max_y_feat = max_y_feat[0].item()
        
        # 5. Map back to Image Coordinates
        # Calculate displacement in the feature space relative to the center
        # (This is a simplified mapping logic)
        
        # The top-left of the score map corresponds to the top-left of the valid correlation area
        offset_x = max_x_feat * self.stride
        offset_y = max_y_feat * self.stride
        
        # Update bounding box
        new_x = search_x1 + offset_x
        new_y = search_y1 + offset_y
        
        self.last_bbox = (new_x, new_y, w, h)
        
        # Simple confidence check (optional)
        confidence = score_map.max().item()
        success = confidence > 15.0 # Threshold depends on feature magnitude
        
        return success, self.last_bbox

# ==========================================
# Main Tracker Class (Modified)
# ==========================================
...
class ObjectTracker:
    def __init__(self):
        ...
        # Priority: Siamese (Deep Learning) -> GPU Template -> CSRT
        if "siamese_resnet" in self.OPENCV_OBJECT_TRACKERS:
            self.tracker_name = "siamese_resnet"
        elif "gpu_template" in self.OPENCV_OBJECT_TRACKERS:
            self.tracker_name = "gpu_template"
        elif "csrt" in self.OPENCV_OBJECT_TRACKERS:
            self.tracker_name = "csrt"
        else:
            self.tracker_name = list(self.OPENCV_OBJECT_TRACKERS.keys())[0]
        ...
        # New: Instance for deep learning tracker
        self.siamese_tracker = None
    ...
    def _get_available_trackers(self):
        ...
        if TORCH_AVAILABLE:
            trackers["siamese_resnet"] = "One-Shot_Siamese_ResNet"
        ...
        if GPUTIL_AVAILABLE:
            try:
                ...
                if gpus:
                    ...
                    self.gpu_stats = {
                        "load": gpu.load * 100,
                        "memory": f"{int(gpu.memoryUsed)}/{int(gpu.memoryTotal)}MB",
    ...
    def initialize_tracker(self, frame, roi):
        ...
        if self.tracker_name == "siamese_resnet":
            print("Initializing Deep Siamese Network...")
            self.use_gpu = torch.cuda.is_available()
            self.siamese_tracker = DeepSiameseTracker(use_gpu=self.use_gpu)
            self.siamese_tracker.init(frame, roi)
            
        elif self.tracker_name == "gpu_template":
            self.use_gpu = True
    ...
    def track_object(self, frame):
        ...
        if self.tracker_name == "siamese_resnet":
            success, bbox = self.siamese_tracker.update(frame)
        
        elif self.tracker_name == "gpu_template" and self.use_gpu:
    ...
    def _track_template(self, frame):
        ...
        if max_val > 0.6:
            ...
            # Slow update of template (Drift handling)
            ...
            if new_template.shape == self.template.shape:
                self.template = cv2.addWeighted(self.template, 0.9, new_template, 0.1, 0)
...
def main():
    print("=" * 60)
    print("Real-Time One-Shot Object Tracker")
    print("=" * 60)
    ...
    if tracker.gpu_available:
        ...
        try:
            ...
            # Warm up PyTorch CUDA if available
